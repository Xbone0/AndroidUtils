...
public class CalendarUtil {
    ...
    public static final int[] getDate() {
        ...
        //这里的month要+1
        date[1] = today.get(Calendar.MONTH) + 1;
     ...
     *
     * @param date
     * @param hanzi 是否将日期转为汉字
     * @return
     */
    public static final String dateToString(int[] date, boolean hanzi) {
        NumberUtil util = NumberUtil.getInstance();
        String mon = "";
        String day = "";
        if (hanzi) {
            mon = util.numToHanzi(date[0]) + "月";
            day = util.numToHanzi(date[1]) + "日";
        } else {
            mon = date[0] + "月";
            day = date[1] + "日";
        }
